...
| VECTOR_DB_PROVIDER       | Default vector DB provider used when a site has no per-site config. One of `qdrant\|opensearch\|elasticsearch\|pinecone\|weaviate\|milvus\|chroma\|pgvector`. Default: `qdrant`        |
...
| OPENSEARCH_ENDPOINT      | OpenSearch endpoint URL (used when VECTOR_DB_PROVIDER=opensearch). For AWS OpenSearch Serverless (AOSS, `*.aoss.<region>.on.aws`) use the AOSS endpoint — region/service are parsed from the URL and SigV4 is signed automatically. |
| OPENSEARCH_USERNAME      | OpenSearch username. For AOSS: AWS Access Key ID                                                                                                                                                           |
| OPENSEARCH_PASSWORD      | OpenSearch password. For AOSS: AWS Secret Access Key                                                                                                                                                       |
