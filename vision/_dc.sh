...
# docker compose는 .env의 COMPOSE_FILE과 COMPOSE_PROFILES를 자동으로 읽는다.
SUDO=""
if ! docker info >/dev/null 2>&1; then
  if [ "$(id -u)" -ne 0 ] && command -v sudo >/dev/null 2>&1 && sudo docker info >/dev/null 2>&1; then SUDO="sudo "
  else echo "Cannot access the Docker daemon. Check daemon status and permissions." >&2; return 1 2>/dev/null || exit 1; fi
fi
...
else echo "docker compose plugin or docker-compose is required." >&2; return 1 2>/dev/null || exit 1; fi
